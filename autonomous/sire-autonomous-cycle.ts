...
async function getStoredSireHistory(){
  const response=await fetch(SIRE_PUBLIC_URL+'/api/sire/deriv/history',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({symbol:SYMBOL,count:COUNT,granularity:60,end:'latest'})});
  const raw=await response.text(); let data:any={}; try{data=raw?JSON.parse(raw):{};}catch{}
  ...
  return {source:'SIRE persistent market history',endpoint:SIRE_PUBLIC_URL+'/api/sire/deriv/history',symbol:SYMBOL,timeframe:INTERVAL,candleCount:candles.length,latestCandle:candles.at(-1),previousCandle:candles.at(-2),observedAt:new Date().toISOString(),candles};
}

async function getMarketObservation(){let lastError='Unknown Deriv market-data error';try{return await getStoredSireHistory();}catch(e){lastError=e instanceof Error?e.message:String(e);}try{return await getHttpHistory();}catch(e){lastError=e instanceof Error?e.message:String(e);}try{return await getCorsHistory();}catch(e){lastError=e instanceof Error?e.message:String(e);}try{return await getJinaHistory();}catch(e){lastError=e instanceof Error?e.message:String(e);}try{return await getFromWs(SIRE_PUBLIC_URL.replace(/^http/,'ws')+'/deriv/ws');}catch(e){lastError=e instanceof Error?e.message:String(e);}for(const endpoint of DIRECT_DERIV_WS){try{return await getFromWs(endpoint);}catch(e){lastError=e instanceof Error?e.message:String(e);}}throw new Error('All Deriv public market-data paths failed: '+lastError);}
