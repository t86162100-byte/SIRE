import { analyzeMarketIntelligence, type MarketBar } from './market-intelligence.ts';
...
async function getCorsHistory(){
  const target='https://api.deriv.com/api/v1/ticks_history?symbol='+encodeURIComponent(SYMBOL)+'&end=latest&count='+COUNT+'&style=candles&granularity=60';
  const response=await fetch('https://proxy.cors.dev/'+target);
  const raw=await response.text();
  if(!response.ok)throw new Error('cors.dev HTTP '+response.status+' '+raw.slice(0,200));
  let data:any={};try{data=JSON.parse(raw);}catch{throw new Error('cors.dev returned non-JSON data: '+raw.slice(0,200));}
  const rawCandles=data?.candles||data?.history?.candles||data?.data?.candles||[];
  const candles=Array.isArray(rawCandles)?rawCandles.map((c:any)=>({epoch:Number(c.epoch),open:Number(c.open),high:Number(c.high),low:Number(c.low),close:Number(c.close)})).filter((c:any)=>Object.values(c).every(Number.isFinite)):[];
  if(candles.length<30)throw new Error('cors.dev returned fewer than 30 valid candles: '+candles.length);
  return {source:'Deriv public market data via cors.dev proxy',endpoint:target,symbol:SYMBOL,timeframe:INTERVAL,candleCount:candles.length,latestCandle:candles.at(-1),previousCandle:candles.at(-2),observedAt:new Date().toISOString(),candles};
}

async function getStoredSireHistory(granularity=60, count=COUNT){
  const response=await fetch(SIRE_PUBLIC_URL+'/api/sire/deriv/history',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({symbol:SYMBOL,count,granularity,end:'latest'})});
  const raw=await response.text(); let data:any={}; try{data=raw?JSON.parse(raw):{};}catch{}
  if(!response.ok)throw new Error('SIRE history endpoint HTTP '+response.status+' '+(data.error||raw.slice(0,200)));
  const candles=Array.isArray(data?.candles)?data.candles.map((c:any)=>({epoch:Number(c.epoch),open:Number(c.open),high:Number(c.high),low:Number(c.low),close:Number(c.close)})).filter((c:any)=>Object.values(c).every(Number.isFinite)):[];
  if(candles.length<30)throw new Error('SIRE persistent history returned fewer than 30 valid candles: '+candles.length);
  return {source:'SIRE persistent market history',endpoint:SIRE_PUBLIC_URL+'/api/sire/deriv/history',symbol:SYMBOL,timeframe:granularity===60?'1m':granularity===300?'5m':granularity===900?'15m':String(granularity)+'s',candleCount:candles.length,latestCandle:candles.at(-1),previousCandle:candles.at(-2),observedAt:new Date().toISOString(),candles};
}

async function getMarketObservation(){let lastError='Unknown Deriv market-data error';try{return await getStoredSireHistory(60,COUNT);}catch(e){lastError=e instanceof Error?e.message:String(e);}try{return await getHttpHistory();}catch(e){lastError=e instanceof Error?e.message:String(e);}try{return await getCorsHistory();}catch(e){lastError=e instanceof Error?e.message:String(e);}try{return await getJinaHistory();}catch(e){lastError=e instanceof Error?e.message:String(e);}try{return await getFromWs(SIRE_PUBLIC_URL.replace(/^http/,'ws')+'/deriv/ws');}catch(e){lastError=e instanceof Error?e.message:String(e);}for(const endpoint of DIRECT_DERIV_WS){try{return await getFromWs(endpoint);}catch(e){lastError=e instanceof Error?e.message:String(e);}}throw new Error('All Deriv public market-data paths failed: '+lastError);}
...
async function updateMemory(state:any){const response=await fetch(GITHUB_API+'/repos/'+REPO+'/issues/'+ISSUE_NUMBER,{method:'PATCH',headers:{Authorization:'Bearer '+TOKEN,Accept:'application/vnd.github+json','X-GitHub-Api-Version':'2022-11-28','Content-Type':'application/json','User-Agent':'SIRE-autonomous-agent'},body:JSON.stringify({body:['SIRE AUTONOMOUS MEMORY — maintained by the same SIRE AI used by the chat tab.','','State: '+state.status,'Last autonomous cycle: '+state.observedAt,'Instrument: '+state.market.symbol,'Timeframe: '+state.market.timeframe,'','## SIRE observation',state.aiObservation||'(no AI observation)','','## Raw market state',JSON.stringify({latestCandle:state.market.latestCandle,previousCandle:state.market.previousCandle,candleCount:state.market.candleCount,source:state.market.source,endpoint:state.market.endpoint||null,marketIntelligence:state.marketIntelligence||null,multiTimeframeIntelligence:state.multiTimeframeIntelligence||null},null,2),'','## Runtime',JSON.stringify({agent:'SIRE',mode:'autonomous',model:state.model||null,provider:state.provider||null,cycleId:state.cycleId,cycleSucceeded:state.status==='healthy'},null,2)].join('\n')})});if(!response.ok)throw new Error('GitHub autonomous memory update failed: '+response.status+' '+(await response.text()).slice(0,500));}
export async function runAutonomousCycle(){const cycleId='sire-auto-'+Date.now();const market=await getMarketObservation();const primaryIntelligence=analyzeMarketIntelligence(market.candles as MarketBar[],200);let multiTimeframe:any[]=[{timeframe:market.timeframe,intelligence:primaryIntelligence}];for(const tf of [{name:'5m',seconds:300},{name:'15m',seconds:900}]){try{const higher=await getStoredSireHistory(tf.seconds,100);multiTimeframe.push({timeframe:tf.name,intelligence:analyzeMarketIntelligence(higher.candles as MarketBar[],100)});}catch(error){multiTimeframe.push({timeframe:tf.name,available:false,error:error instanceof Error?error.message:String(error)});}}const prompt=['Run one autonomous SIRE market-intelligence observation cycle.','You are the SAME SIRE AI that users chat with in the SIRE tab, not a separate observer.','The deterministic engine has already calculated the measurable market conditions below. Interpret those results; do not recalculate or invent values.','Do not create a trading strategy yet. Do not make a trade recommendation.','Separate direct observations from uncertainty or unavailable data.','Instrument: '+market.symbol,'Deterministic 1m intelligence: '+JSON.stringify(primaryIntelligence),'Deterministic multi-timeframe intelligence: '+JSON.stringify(multiTimeframe),'Return a concise factual observation that can be read later by the same SIRE chat agent.'].join('\n');const chartSnapshot={source:'autonomous SIRE market engine',autonomous:true,symbol:market.symbol,timeframe:market.timeframe,recentBars:market.candles,marketIntelligence:primaryIntelligence,multiTimeframeIntelligence:multiTimeframe,autonomousSamples:[{observedAt:market.observedAt,latestCandle:market.latestCandle}],liveMarketData:{connectionStatus:'connected',subscriptionStatus:'historical_snapshot',latestTick:market.latestCandle?.close??null,dataTimestamp:market.latestCandle?.epoch??null,stale:false,staleThresholdMs:120000}};const ai=await askSameSireAi(prompt,chartSnapshot);const state={status:'healthy',cycleId,observedAt:market.observedAt,market,marketIntelligence:primaryIntelligence,multiTimeframeIntelligence:multiTimeframe,aiObservation:ai.text,model:ai.model,provider:ai.provider};await updateMemory(state);console.log(JSON.stringify({service:'sire-autonomous-agent',event:'autonomous.cycle.completed',cycleId,observedAt:market.observedAt,symbol:market.symbol,model:ai.model,provider:ai.provider}));return state;}
