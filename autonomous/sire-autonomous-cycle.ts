...
async function getMarketObservation(){let lastError='Unknown Deriv market-data error';try{return await getHttpHistory();}catch(e){lastError=e instanceof Error?e.message:String(e);}try{return await getCorsHistory();}catch(e){lastError=e instanceof Error?e.message:String(e);}try{return await getJinaHistory();}catch(e){lastError=e instanceof Error?e.message:String(e);}try{return await getFromWs(SIRE_PUBLIC_URL.replace(/^http/,'ws')+'/deriv/ws');}catch(e){lastError=e instanceof Error?e.message:String(e);}for(const endpoint of DIRECT_DERIV_WS){try{return await getFromWs(endpoint);}catch(e){lastError=e instanceof Error?e.message:String(e);}}throw new Error('All Deriv public market-data paths failed: '+lastError);}
