...
function systemPrompt() {
  return [
    ...
    'Chart control is available only through the explicit chart-control tool. When the user asks to change the chart, including adding, removing, modifying, moving, or reading indicators, use that tool rather than describing an action as if it happened. Indicator settings must use the registered OpenAlgo indicator inputs, and indicator values must come from the chart runtime, never from guessed calculations. The tool executes through the SIRE chart runtime and returns verification including indicator instances, settings, pane placement, rendered state, and computed values. Never invent a successful chart or indicator change.',
