...
export async function runGptHead(input: {
  ...
  if (input.tools?.chartControl) toolDefs.push({ type:'function', function:{ name:'control_chart', description:'Operate the active SIRE chart through its controlled chart-runtime interface, then return verified chart state. Use when the user asks to switch instrument, switch timeframe, change chart type, zoom, pan, move to a specific time, reset view, fit chart to data, open/close chart panes, open chart settings, add/remove/modify/move/read indicators, or add/remove/modify/read drawings. IMPORTANT: common Deriv instrument names map directly to symbols: BOOM 1000 = BOOM1000, CRASH 1000 = CRASH1000, etc. Time phrases map to chart intervals: 1-minute = 1m, 5-minute = 5m, 15-minute = 15m, 30-minute = 30m, 1-hour = 1h. For indicators, use the registered OpenAlgo descriptor placement: onchart indicators use price pane 0; pane indicators (including MACD/RSI/Stochastic/ADX/ATR) must use a separate pane. For a new pane indicator, omit paneIndex; do not pass paneIndex 0. For drawings, supported registered tools include trend-line, horizontal-line, vertical-line, ray, parallel-channel, rectangle, fib-retracement and text. Before any coordinate-based drawing creation, call inspect_drawing_context first and use its returned real time/price anchors. If a separate inspection call is needed, call control_chart again; it is safe and expected to call control_chart more than once in one request. The registered anchor counts are: horizontal-line/vertical-line=1, trend-line/ray/fib-retracement/rectangle=2, parallel-channel=3. Never invent drawing coordinates. This is the only supported write interface to the visual chart. Do not claim success without using this tool and reading its verification result.', parameters:{type:'object',properties:{operations:{type:'array',minItems:1,maxItems:10,items:{type:'object',properties:{action:{type:'string',enum:['switch_instrument','switch_timeframe','set_chart_type','zoom','pan','move_to_time','reset_view','fit_data','open_pane','close_pane','open_settings','set_theme','set_timezone','set_grid','set_price_scale','set_crosshair','add_indicator','remove_indicator','modify_indicator','move_indicator','set_indicator_visibility','read_indicators','inspect_drawing_context','add_drawing','remove_drawing','modify_drawing','read_drawings']},symbol:{type:'string'},name:{type:'string'},interval:{type:'string'},chartType:{type:'string'},direction:{type:'string',enum:['in','out','left','right']},factor:{type:'number'},bars:{type:'number'},time:{type:'number'},paneIndex:{type:'integer'},open:{type:'boolean'},theme:{type:'string',enum:['dark','light']},timezone:{type:'string'},settings:{type:'object',additionalProperties:true},indicatorId:{type:'string'},instanceId:{type:'string'},drawingId:{type:'string'},drawingTool:{type:'string'},points:{type:'array',items:{type:'object',properties:{time:{type:'number'},price:{type:'number'}},required:['time','price'],additionalProperties:false}},paneIndex:{type:'integer'},visible:{type:'boolean'},locked:{type:'boolean'},text:{type:'object',additionalProperties:true},style:{type:'object',additionalProperties:true},props:{type:'object',additionalProperties:true},zIndex:{type:'number'},fromTime:{type:'number'},toTime:{type:'number'},maxBars:{type:'integer',minimum:1,maximum:300}},required:['action'],additionalProperties:false}}},required:['operations'],additionalProperties:false} } });
