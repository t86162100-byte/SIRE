...
function systemPrompt() {
  return [
    ...
    'Chart control is available only through the explicit chart-control tool. When the user asks to change the chart, including adding, removing, modifying, moving, or reading indicators, use that tool rather than describing an action as if it happened. Indicator settings must use the registered OpenAlgo indicator inputs, and indicator values must come from the chart runtime, never from guessed calculations. For indicator placement, trust the registered descriptor placement: onchart indicators belong on price pane 0; pane indicators such as MACD, RSI, Stochastic, ADX and ATR must be in a separate indicator pane. When adding a pane indicator, OMIT paneIndex unless the user explicitly requests an existing non-price pane; NEVER send paneIndex 0 for a pane indicator. The tool executes through the SIRE chart runtime and returns verification including indicator instances, settings, pane placement, rendered state, and computed values. Never invent a successful chart or indicator change.',
...
export async function runGptHead(input: {
  ...
  const chartIntent = (/\b(open|switch|change|set|show|load|go to|move|zoom|pan|reset|fit|add|remove|delete|modify|edit|change|put)\b/i.test(query) && /\b(chart|instrument|market|timeframe|candle|candlestick|minute|hour|indicator|indicators|ema|sma|wma|rsi|macd|bollinger|adx|atr|vwap|stochastic|BOOM|CRASH)\b/i.test(query)) || /\b(BOOM|CRASH)\s*\d+\b/i.test(query);
