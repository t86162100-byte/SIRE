...
export async function runGptHead(input: {
  ...
  const chartIntent = (/\b(open|switch|change|set|show|load|go to|move|zoom|pan|reset|fit)\b/i.test(query) && /\b(chart|instrument|market|timeframe|candle|candlestick|minute|hour|BOOM|CRASH)\b/i.test(query)) || /\b(BOOM|CRASH)\s*\d+\b/i.test(query);
