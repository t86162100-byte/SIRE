...
export async function runAiTeam(input:{query:string;workspaceId?:string;history?:any[];symbol?:string;execute?:boolean;onEvent?:TeamEvent}){
 ...
 return lock(id,async()=>{
  ...
  return {diagnostics,text:result.text,responseId:result.responseId||'',model:result.model||'openai/gpt-oss-20b',provider:'OpenAI gpt-oss via OpenRouter',teamMode:'gpt-head',workspaceId:id,responsibilities:s.responsibilities,decisions:s.decisions.slice(-12),openQuestions:s.openQuestions.slice(-12),artifacts:s.artifacts.slice(-8),activity:s.activity.slice(-20),execution:input.execute?{status:'planned'}:{status:'not_requested'},agentSkills:[],agentToolTrace:[],webSearched:Boolean(timings.webSearch),webSources:[],sharedState:stateText(s)};
 });
}
