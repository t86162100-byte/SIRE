...
import { runGptHead } from './openrouter-ai.ts';
 ...
 return lock(id,async()=>{
  ...
  const result=await runGptHead({
    ...
    tools:{
      ...
      webSearch:async(q)=>{
        const t=Date.now();await ev('Web','research','GPT decided that current external information is needed, so SIRE is searching the web.');const r=await webSearch(q,8).catch(()=>({results:[]}));timings.webSearch=Date.now()-t;
  ...
  return {diagnostics,text:result.text,responseId:result.responseId||'',model:result.model||'openai/gpt-oss-20b',provider:'OpenAI gpt-oss via OpenRouter',teamMode:'gpt-head',workspaceId:id,responsibilities:s.responsibilities,decisions:s.decisions.slice(-12),openQuestions:s.openQuestions.slice(-12),artifacts:s.artifacts.slice(-8),activity:s.activity.slice(-20),execution:input.execute?{status:'planned'}:{status:'not_requested'},actions:[],agentActions:[],agentSkills:[],agentToolTrace:[],webSearched:Boolean(timings.webSearch),webSources:[],sharedState:stateText(s)};
 });
}
