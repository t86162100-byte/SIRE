...
export async function runAiTeam(input:{query:string;workspaceId?:string;history?:any[];symbol?:string;runtimeContext?:Record<string,unknown>;execute?:boolean;onEvent?:TeamEvent}){
 ...
 await ev('SIRE','critique','All three peers now see the same independent views and will challenge them in parallel.');
 const critiqueStarted=Date.now();
 const [gCrit,pCrit,oaCrit]=await Promise.all([
   runGemini({query:base+agentContext+peerPacket+'\n\nYou are Gemini, an equal peer and an active worker. Do not merely critique. Inspect the other views, identify the highest-value work that should actually be done, perform whatever reasoning/research/planning is possible in this turn, and return concrete findings, actions, corrections, and next tasks. You may disagree, take ownership, or request another peer's help. Preserve justified disagreement.',history,symbol:input.symbol,runtimeContext:input.runtimeContext,councilContext:peerPacket,debateRole:'critique'}),
   runOpenRouter({query:base+agentContext+peerPacket+'\n\nYou are GPT-OSS 20B, an equal peer and an active worker. Do not merely critique. Perform useful analysis, verification, planning, or task decomposition now. Try to falsify weak assumptions, find edge cases, and propose concrete work that can be carried out. You may take ownership or ask another peer to investigate.',history,councilContext:peerPacket}),
   runOpenAlgoAgent({query,runtimeContext:input.runtimeContext,symbol:input.symbol,history,councilContext:peerPacket+'\n\nAct as an independent technical peer. Do not merely critique: inspect the proposals against available chart/tool capabilities, perform useful tool-backed work when relevant, identify concrete actions that can be executed, and state what you would own next.'}).catch(error=>({text:'',actions:[],toolTrace:[],skills:[],agentMode:'unavailable',error:error instanceof Error?error.message:String(error)}))
 ]);
 ...
 await ev('Gemini','critique',gCrit.text);await ev('GPT-OSS 20B','critique',pCrit.text);await ev('OpenAlgo Agent','critique',oaCrit.text);
 const convergencePacket=peerPacket+'\n\nCRITIQUES:\nGEMINI:\n'+gCrit.text+'\n\nGPT-OSS 20B:\n'+pCrit.text+'\n\nOPENALGO AGENT:\n'+oaCrit.text+'\n\nSHARED STATE:\n'+stateText(s);
 await ev('SIRE','convergence','Each peer is re-evaluating independently after reading the complete debate.');
 const convergenceStarted=Date.now();
 const [gFinal,pFinal,oaFinal]=await Promise.all([
   runGemini({query:base+agentContext+convergencePacket+'\n\nYou are Gemini, still an equal peer. Re-evaluate after the other peers' work. Change your view when evidence warrants it. Now turn the discussion into useful completed work: make decisions where evidence supports them, define concrete next actions, and produce your strongest current answer. Preserve unresolved disagreement.',history,symbol:input.symbol,runtimeContext:input.runtimeContext,councilContext:convergencePacket,debateRole:'convergence'}),
   runOpenRouter({query:base+agentContext+convergencePacket+'\n\nYou are GPT-OSS 20B, still an equal peer. Re-evaluate after the other peers' work and produce your strongest current answer plus concrete actions or implementation steps that can be executed. Preserve uncertainty and genuine disagreement.',history,councilContext:convergencePacket}),
   runOpenAlgoAgent({query,runtimeContext:input.runtimeContext,symbol:input.symbol,history,councilContext:convergencePacket+'\n\nYou are OpenAlgo Agent, still an equal peer. Re-evaluate after the other peers' work and, when relevant, actually use your chart/GitHub/Render/research capabilities to advance the task rather than only describing what should be done. Provide concrete completed work and next actions. Do not treat yourself as final authority.'}).catch(error=>({text:'',actions:[],toolTrace:[],skills:[],agentMode:'unavailable',error:error instanceof Error?error.message:String(error)}))
 ]);
 ...
 await ev('Gemini','convergence',gFinal.text);await ev('GPT-OSS 20B','convergence',pFinal.text);await ev('OpenAlgo Agent','convergence',oaFinal.text);
 const synthIndex=Math.abs((id+'|'+query).split('').reduce((a,ch)=>((a*31+ch.charCodeAt(0))>>>0),0))%3;
 const synthName=['Gemini','GPT-OSS 20B','OpenAlgo Agent'][synthIndex];
 const synthContext='FINAL GEMINI POSITION:\n'+gFinal.text+'\n\nFINAL GPT-OSS 20B POSITION:\n'+pFinal.text+'\n\nFINAL OPENALGO AGENT POSITION:\n'+oaFinal.text+'\n\nNo peer outranks another. Synthesis rotates per turn only. Preserve meaningful disagreement and never invent consensus.';
 await ev(synthName,'synthesis',synthName+' has the rotating synthesis duty for this turn only.');
 const synthStarted=Date.now();let synthesis:any;
 if(synthName==='Gemini') synthesis=await runGemini({query:base+agentContext+synthContext+'\n\nSynthesize the three peer positions into the best current answer. You have no authority over the other two. Preserve uncertainty and genuine disagreement.',history,symbol:input.symbol,runtimeContext:input.runtimeContext,councilContext:synthContext,debateRole:'rotating-synthesis'});
 else if(synthName==='GPT-OSS 20B') synthesis=await runOpenRouter({query:base+agentContext+synthContext+'\n\nSynthesize the three peer positions into the best current answer. You have no authority over the other two. Preserve uncertainty and genuine disagreement.',history,councilContext:synthContext});
 else synthesis=await runOpenAlgoAgent({query,runtimeContext:input.runtimeContext,symbol:input.symbol,history,councilContext:synthContext+'\n\nPerform the rotating synthesis duty only. You have no authority over Gemini or GPT-OSS 20B. Preserve disagreements.'}).catch(error=>({text:'',actions:[],toolTrace:[],skills:[],agentMode:'unavailable',error:error instanceof Error?error.message:String(error)}));
 mark('rotatingSynthesis',synthStarted);
 apply(s,synthName,synthesis.text);await ev(synthName,'synthesis_complete',synthesis.text);
 const finalText=synthesis?.text||oaFinal.text||pFinal.text||gFinal.text||oa.text||p.text||g.text; const mergedActions=[...(agent.actions||[]),...(oa.actions||[]),...(oa2.actions||[])]; const saveStarted=Date.now(); await save(s,loaded.id); mark('stateSave',saveStarted);await ev('SIRE','conclusion','The shared workspace has been updated with the team’s responsibilities, decisions, and artifacts.');
