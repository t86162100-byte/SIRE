...
export async function runAiTeam(input:{query:string;workspaceId?:string;history?:any[];symbol?:string;runtimeContext?:Record<string,unknown>;execute?:boolean;onEvent?:TeamEvent}){
 ...
 await ev('Chief AI','reviewing','The Chief AI is reviewing both teammates, checking the shared state, and resolving any real disagreement.');const chiefReviewStarted=Date.now(); const chief=await runGemini({query:`${base}${agentContext}\n\nSHARED TEAM STATE:\n${stateText(s)}\n\nGEMINI CONTRIBUTION:\n${g.text}\n\nGPT CONTRIBUTION:\n${p.text}\n\nYou are the Chief AI. You are the third member of the team and the final coordinator, not a passive router. Review both teammates' work, identify agreements and genuine disagreements, correct weak assumptions, decide what each teammate should own next, and make the joint decision. You may side with Gemini, side with GPT, combine their ideas, or reject both when the evidence requires it. Do not manufacture disagreement. Return a concise visible team summary followed by JSON {\\\"summary\\\":\\\"...\\\",\\\"responsibilities\\\":[...],\\\"decisions\\\":[\\\"...\\\"],\\\"openQuestions\\\":[\\\"...\\\"],\\\"artifacts\\\":[]}. Never expose hidden chain-of-thought.`,history,symbol:input.symbol,runtimeContext:input.runtimeContext,councilContext:`Gemini:\n${g.text}\n\nGPT:\n${p.text}`,debateRole:'response'});mark('chiefReview',chiefReviewStarted); apply(s,'Chief AI',chief.text);await ev('Chief AI','decision',chief.text);
 await ev('GPT','executing','GPT is taking the Chief AI decision and carrying out its assigned responsibility.');const gptFinalStarted=Date.now(); const f=await runOpenRouter({query:`${base}${agentContext}\n\nFINAL SHARED STATE:\n${stateText(s)}\n\nCHIEF AI DECISION:\n${chief.text}\n\nGEMINI CONTRIBUTION:\n${g.text}\n\nGPT CONTRIBUTION:\n${p.text}\n\nYou are the execution teammate. Follow the Chief AI decision unless you have a concrete evidence-based reason to flag a problem. Produce the direct answer and concrete next action. If external execution is requested, distinguish actions actually confirmed by SIRE tools from actions still requiring authorization. Return a JSON object containing {\\\"userAnswer\\\":\\\"...\\\",\\\"summary\\\":\\\"...\\\",\\\"responsibilities\\\":[...],\\\"decisions\\\":[\\\"...\\\"],\\\"openQuestions\\\":[\\\"...\\\"],\\\"artifacts\\\":[...]}. The userAnswer is the only user-facing answer and must contain no team-state JSON or internal protocol. Never claim an external action occurred without tool confirmation.`,history,councilContext:`CHIEF AI DECISION:\n${chief.text}\n\nSHARED STATE:\n${stateText(s)}`});mark('gptFinal',gptFinalStarted); apply(s,'GPT',f.text); const saveStarted=Date.now(); await save(s,loaded.id); mark('stateSave',saveStarted);await ev('SIRE','conclusion','The shared workspace has been updated with the team’s responsibilities, decisions, and artifacts.');
