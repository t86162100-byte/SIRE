  ...
  for (const model of MODELS) {
    try {
      for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
        const requestBody = { system_instruction: { parts: [{ text: systemInstruction }] }, contents, tools: HEAD_TOOLS, tool_config: { function_calling_config: { mode: 'AUTO' } }, generationConfig: { temperature: 0.7, maxOutputTokens: 2048 } };
