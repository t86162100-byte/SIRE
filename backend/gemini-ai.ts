...
export async function runGemini(input: { query: string; history?: Array<{ role: string; text?: string; content?: string }>; councilContext?: string; debateRole?: string; onEvent?: CouncilEvent }) {
  ...
  const systemInstruction = [
    ...
    'When a difficult task would benefit from another model, you may ask GPT-OSS 20B for an independent technical or reasoning pass. When current external information is needed, use web_search.',
