...
export function MobileDrawingPlacement({chartRef,tool,seriesId,label,onCancel,onComplete}:Props){
  ...
  useEffect(()=>{setPlaced([]);setContent('');setReticle(null);},[tool]);
  ...
  const toPoint=useCallback((p:{x:number;y:number})=>{const chart=chartRef.current;if(!chart)return null;const price=chart.coordinateToPrice(p.y);const ts=chart.coordinateToTime(p.x);return price==null||ts==null?null:{price,ts};},[chartRef]);
  const finish=useCallback((points:DrawingPoint[],text?:string)=>{if(!tool)return;const drawing=buildDrawing(tool,points,seriesId,text);if(!drawing)return;chartRef.current?.executeCommand({type:'addDrawing',payload:drawing});setPlaced([]);setContent('');setReticle(null);onComplete();},[chartRef,onComplete,seriesId,tool]);
  const confirmPoint=useCallback(()=>{if(!tool||!needed)return;const point=toPoint(cursor);if(!point)return;const next=[...placed,point];if(next.length<needed){setPlaced(next);return;}if(tool==='text'||tool==='callout'){setPlaced(next);return;}finish(next);},[cursor,finish,needed,placed,toPoint,tool]);
  ...
  const anchors=placed.flatMap(item=>{const x=chartRef.current?.timeToCoordinate(item.ts);const y=chartRef.current?.priceToCoordinate(item.price);return x==null||y==null?[]:[{x,y}];});
  const guide=[...anchors,cursor];
  return <div ref={hostRef} className="sire-mobile-placement-layer" style={{position:'absolute',inset:0,zIndex:10020,pointerEvents:'none'}}>
    <svg aria-hidden style={{position:'absolute',inset:0,width:'100%',height:'100%',pointerEvents:'none'}} viewBox={`0 0 ${Math.max(frame.width,1)} ${Math.max(frame.height,1)}`}>{guide.length>1?<polyline fill="none" points={guide.map(p=>`${p.x},${p.y}`).join(' ')} stroke="#3ea6ff" strokeWidth="1.5" strokeDasharray="5 4"/>:null}</svg>
