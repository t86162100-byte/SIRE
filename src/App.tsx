...
export default function App() {
  ...
  const selectInstrument = (item: DerivInstrument) => { setSelected(item); setInstrumentMenuOpen(false); setSearch(''); };
  ...
  return (
    <main className="native-terminal-shell">
      ...
      {instrumentMenuOpen && <div className="native-instrument-overlay" onClick={() => setInstrumentMenuOpen(false)}><div className="native-instrument-sheet" onClick={event => event.stopPropagation()}><div className="sheet-head"><div><span>CHANGE INSTRUMENT</span><b>{instruments.length} Synthetic Indices</b></div><button onClick={() => setInstrumentMenuOpen(false)}>Done</button></div><div className="sheet-search"><Search size={15} /><input autoFocus value={search} onChange={event => setSearch(event.target.value)} placeholder="Search instrument or symbol" /></div><div className="sheet-list">{filtered.map(item => <button key={item.symbol} className={`sheet-row ${selected?.symbol === item.symbol ? 'active' : ''}`} onClick={() => selectInstrument(item)}><span><b>{item.name}</b><small>{item.symbol}</small></span><em>{selected?.symbol === item.symbol ? 'SELECTED' : item.exchangeOpen === 0 ? 'OFF' : 'LIVE'}</em></button>)}{!filtered.length && <div className="empty-state">No Synthetic Indices found.</div>}</div></div></div>}
      ...
      <div className="native-terminal-body">
        ...
        <section className="native-chart-panel"><div className="native-chart-toolbar"><div className="native-timeframes" /><div className="native-chart-actions"><button title="Open research laboratory" onClick={() => setResearchLabOpen(true)}><Beaker size={14} /></button></div></div><div className="native-chart-subbar"><div><b>{selected?.name || 'No instrument'}</b><span>{selected?.symbol || '—'}</span></div><div className="native-ohlc"><span>Bid <b>{latest?.bid !== undefined ? latest.bid.toLocaleString(undefined, { maximumFractionDigits: 8 }) : '—'}</b></span><span>Ask <b>{latest?.ask !== undefined ? latest.ask.toLocaleString(undefined, { maximumFractionDigits: 8 }) : '—'}</b></span></div></div>{lastError && <div className="native-error-banner">{lastError}</div>}{selected && <FinancialChart symbol={selected.symbol} liveTick={latest} requestHistory={requestHistory} />}</section>
      </div>
