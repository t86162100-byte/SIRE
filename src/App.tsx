...
function textOf(item: Record<string, unknown>, keys: string[]) { return keys.map(key => item[key]).filter(value => value !== undefined && value !== null).map(value => String(value)).join(' ').toLowerCase(); }
function isSynthetic(item: Record<string, unknown>) {
  const market = String(item.market || '').toLowerCase(); const submarket = String(item.submarket || '').toLowerCase(); const subgroup = String(item.subgroup || '').toLowerCase(); const symbol = String(item.underlying_symbol || item.symbol || '');
...
function normalize(item: Record<string, unknown>): Instrument | null {
  const symbol = String(item.underlying_symbol || item.symbol || ''); if (!symbol) return null;
  return { symbol, name: String(item.underlying_symbol_name || item.display_name || symbol), market: String(item.market || ''), submarket: String(item.submarket || ''), subgroup: String(item.subgroup || ''), symbolType: String(item.underlying_symbol_type || item.symbol_type || ''), exchangeOpen: typeof item.exchange_is_open === 'number' ? item.exchange_is_open : undefined };
}
function openDeriv(url: string): Promise<WebSocket> {
  return new Promise((resolve, reject) => { const socket = new WebSocket(url); const timer = window.setTimeout(() => { socket.close(); reject(new Error('Deriv connection timed out')); }, 10000); socket.onopen = () => { window.clearTimeout(timer); resolve(socket); }; socket.onerror = () => { window.clearTimeout(timer); reject(new Error(`Unable to connect to ${url.includes('binaryws') ? 'legacy' : 'current'} Deriv market-data endpoint`)); }; });
}
function requestOnce(ws: WebSocket, request: Record<string, unknown>): Promise<Record<string, unknown>> {
  return new Promise((resolve, reject) => {
    const reqId = Math.floor(Math.random() * 900000000) + 100000000; const timer = window.setTimeout(() => { ws.removeEventListener('message', onMessage); reject(new Error('Deriv request timed out')); }, 12000);
    const onMessage = (event: MessageEvent) => { try { const data = JSON.parse(event.data) as Record<string, unknown>; if (Number(data.req_id) !== reqId) return; window.clearTimeout(timer); ws.removeEventListener('message', onMessage); if (data.error) { const error = data.error as Record<string, unknown>; reject(new Error(String(error.message || 'Deriv API error'))); return; } resolve(data); } catch { window.clearTimeout(timer); ws.removeEventListener('message', onMessage); reject(new Error('Invalid Deriv response')); } };
    ws.addEventListener('message', onMessage); ws.send(JSON.stringify({ ...request, req_id: reqId }));
  });
}
async function discoverFrom(endpoint: string, label: string): Promise<DiscoveryResult> {
  const ws = await openDeriv(endpoint);
  try {
    const request: Record<string, unknown> = { active_symbols: 'brief' }; if (endpoint === LEGACY_DERIV_WS) request.product_type = 'basic';
    const data = await requestOnce(ws, request); const all = Array.isArray(data.active_symbols) ? data.active_symbols : [];
...
async function discoverCatalogue(): Promise<DiscoveryResult> {
  let legacyError = '';
  try { const legacy = await discoverFrom(LEGACY_DERIV_WS, 'Deriv public market-data API'); if (legacy.instruments.length) return legacy; legacyError = `Deriv returned ${legacy.totalMarkets} active markets but no Synthetic Indices matched`; } catch (error) { legacyError = error instanceof Error ? error.message : String(error); }
  try { const current = await discoverFrom(CURRENT_DERIV_WS, 'Deriv current public API fallback'); if (current.instruments.length) return current; throw new Error(`Current API returned ${current.totalMarkets} active markets but no Synthetic Indices matched`); } catch (error) { throw new Error(`${legacyError}; current fallback: ${error instanceof Error ? error.message : String(error)}`); }
}
...
export default function App() {
  const [instruments, setInstruments] = useState<Instrument[]>([]); const [selected, setSelected] = useState<Instrument | null>(null); const [search, setSearch] = useState(''); const [status, setStatus] = useState('Connecting to Deriv…'); const [lastError, setLastError] = useState(''); const [endpointLabel, setEndpointLabel] = useState(''); const [totalMarkets, setTotalMarkets] = useState(0); const [latest, setLatest] = useState<Tick | null>(null); const [chartTick, setChartTick] = useState<Tick | null>(null); const [liveTicks, setLiveTicks] = useState(0); const [connectionNonce, setConnectionNonce] = useState(0); const [instrumentMenuOpen, setInstrumentMenuOpen] = useState(false); const [researchLabOpen, setResearchLabOpen] = useState(false);
  const wsRef = useRef<WebSocket | null>(null); const liveBuffer = useRef<Tick[]>([]); const seen = useRef(new Set<string>());

  const loadCatalog = useCallback(async () => {
    setStatus('Discovering Deriv Synthetic Indices…'); setLastError(''); const result = await discoverCatalogue(); setInstruments(result.instruments); setTotalMarkets(result.totalMarkets); setEndpointLabel(result.endpointLabel);
    try { await api.post('/api/sire/catalogue/sync', { instruments: result.allInstruments }); } catch (error) { setLastError(error instanceof Error ? `Catalogue discovered but persistence failed: ${error.message}` : 'Catalogue discovered but persistence failed'); }
    setSelected(current => current && result.instruments.some(item => item.symbol === current.symbol) ? current : result.instruments[0] || null); setStatus(result.instruments.length ? `Deriv online · ${result.instruments.length} Synthetic Indices discovered` : `Deriv connected · 0 Synthetic Indices · ${result.totalMarkets} active markets inspected`);
  }, []);
  useEffect(() => { void loadCatalog().catch((error: Error) => { setStatus('Deriv connection failed'); setLastError(error.message); }); }, [loadCatalog, connectionNonce]);

  useEffect(() => {
    if (!selected) return;
    let disposed = false; let reconnectTimer = 0; setLatest(null); setChartTick(null); setLiveTicks(0); liveBuffer.current = []; seen.current = new Set();
    const connect = async () => {
      try {
        const ws = await openDeriv(CURRENT_DERIV_WS).catch(() => openDeriv(LEGACY_DERIV_WS)); if (disposed) { ws.close(); return; }
        wsRef.current = ws; setStatus(`Live · ${selected.name}`); ws.send(JSON.stringify({ ticks: selected.symbol, subscribe: 1 }));
        ws.addEventListener('message', event => {
          if (disposed) return;
          try {
            const data = JSON.parse(event.data) as Record<string, unknown>; if (data.error) { const error = data.error as Record<string, unknown>; setLastError(String(error.message || 'Deriv stream error')); return; }
            if (data.msg_type !== 'tick' || !data.tick || typeof data.tick !== 'object') return;
            const raw = data.tick as Record<string, unknown>; const tick: Tick = { symbol: String(raw.symbol || raw.underlying_symbol || selected.symbol), quote: Number(raw.quote), bid: Number.isFinite(Number(raw.bid)) ? Number(raw.bid) : undefined, ask: Number.isFinite(Number(raw.ask)) ? Number(raw.ask) : undefined, epoch: Number(raw.epoch), id: raw.id ? String(raw.id) : undefined, source: 'Deriv' };
            if (!Number.isFinite(tick.quote) || !Number.isFinite(tick.epoch)) return;
            const key = `${tick.symbol}:${tick.epoch}:${tick.quote}`; if (seen.current.has(key)) return; seen.current.add(key); liveBuffer.current.push(tick); setLatest(tick); setChartTick(tick); setLiveTicks(value => value + 1);
        ...
        ws.addEventListener('close', () => { if (!disposed) { setStatus(`Reconnecting · ${selected.name}`); reconnectTimer = window.setTimeout(connect, 1500); } });
      } catch (error) { if (!disposed) { setLastError(error instanceof Error ? error.message : 'Live Deriv stream failed'); reconnectTimer = window.setTimeout(connect, 2500); } }
    };
    void connect(); return () => { disposed = true; window.clearTimeout(reconnectTimer); if (liveBuffer.current.length) void ingestTicks(selected.symbol, liveBuffer.current.splice(0)); wsRef.current?.close(); wsRef.current = null; };
  }, [selected]);

  const filtered = useMemo(() => { const query = search.trim().toLowerCase(); return query ? instruments.filter(item => `${item.name} ${item.symbol}`.toLowerCase().includes(query)) : instruments; }, [instruments, search]);
