  ...
  return new Promise((resolve, reject) => {
    ...
    const onMessage = (event: MessageEvent) => {
      try {
        ...
        if (data.error) { const error = data.error as Record<string, unknown>; reject(new Error(String(error.message || 'Deriv API error'))); return; }
        resolve(data);
      } catch { window.clearTimeout(timer); ws.removeEventListener('message', onMessage); reject(new Error('Invalid Deriv response')); }
    };
...
async function discoverCatalogue(): Promise<DiscoveryResult> {
  ...
  try {
    ...
    if (legacy.instruments.length) return legacy;
    legacyError = `Deriv returned ${legacy.totalMarkets} active markets but no Synthetic Indices matched`;
  } catch (error) { legacyError = error instanceof Error ? error.message : String(error); }
  try {
    const current = await discoverFrom(CURRENT_DERIV_WS, 'Deriv current public API fallback');
    if (current.instruments.length) return current;
    throw new Error(`Current API returned ${current.totalMarkets} active markets but no Synthetic Indices matched`);
  } catch (error) { throw new Error(`${legacyError}; current fallback: ${error instanceof Error ? error.message : String(error)}`); }
}
async function ingestTicks(symbol: string, ticks: Tick[]) {
  for (let i = 0; i < ticks.length; i += INGEST_CHUNK) await api.post('/api/sire/ingest', { symbol, source: 'Deriv', ticks: ticks.slice(i, i + INGEST_CHUNK) });
}
...
export default function App() {
  ...
  const loadCatalog = useCallback(async () => {
    ...
    setInstruments(result.instruments); setTotalMarkets(result.totalMarkets); setEndpointLabel(result.endpointLabel);
    try { await api.post('/api/sire/catalogue/sync', { instruments: result.allInstruments }); }
    catch (error) { setLastError(error instanceof Error ? `Catalogue discovered but persistence failed: ${error.message}` : 'Catalogue discovered but persistence failed'); }
    setSelected(current => current && result.instruments.some(item => item.symbol === current.symbol) ? current : result.instruments[0] || null);
    setStatus(result.instruments.length ? `Deriv online · ${result.instruments.length} Synthetic Indices discovered` : `Deriv connected · 0 Synthetic Indices · ${result.totalMarkets} active markets inspected`);
  }, []);
  ...
  useEffect(() => {
    ...
    setLatest(null); setLiveTicks(0); liveBuffer.current = []; seen.current = new Set();
    const connect = async () => {
      try {
        ...
        ws.addEventListener('message', event => {
          ...
          try {
            ...
            const key = `${tick.symbol}:${tick.epoch}:${tick.quote}`; if (seen.current.has(key)) return; seen.current.add(key);
            liveBuffer.current.push(tick); setLatest(tick); setLiveTicks(value => value + 1);
    ...
    void connect();
    return () => { disposed = true; window.clearTimeout(reconnectTimer); if (liveBuffer.current.length) void ingestTicks(selected.symbol, liveBuffer.current.splice(0)); wsRef.current?.close(); wsRef.current = null; };
  }, [selected]);
    ...
    <>
      <main className="terminal-shell">
        <header className="terminal-topbar">
          ...
          <button className="instrument-picker" onClick={() => setInstrumentMenuOpen(value => !value)} aria-label="Change instrument"><span><strong>{selected?.name || 'Select instrument'}</strong><small>{selected?.symbol || 'Synthetic Index'} · Deriv</small></span><ChevronDown size={16} /></button>
        ...
        <div className="terminal-body">
          ...
          <section className="market-workspace" aria-label="Market workspace"><div className="market-summary"><div><span>SELECTED MARKET</span><strong>{selected?.name || 'No instrument selected'}</strong><small>{selected?.symbol || '—'}</small></div><div><span>LAST QUOTE</span><strong>{latest ? formatQuote(latest.quote) : '—'}</strong><small>{latest ? new Date(latest.epoch * 1000).toLocaleString() : 'Waiting for live data'}</small></div><div><span>LIVE UPDATES</span><strong>{liveTicks.toLocaleString()}</strong><small>{endpointLabel || 'Deriv public market-data'}</small></div></div>{lastError && <div className="terminal-error">{lastError}</div>}<div className="market-empty-state"><Activity size={28} /><b>Market data is ready</b><span>SIRE can inspect instruments and work with their persisted Deriv data.</span><button className="lab-launch" onClick={() => setResearchLabOpen(true)}><Beaker size={13} /> OPEN SIRE RESEARCH</button></div></section>
        </div>
      </main>
