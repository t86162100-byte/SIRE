  ...
  for (const [endpoint, label] of endpoints) {
    try {
      ...
      try {
        ...
        if (instruments.length) return { instruments, allInstruments: records };
      ...
      <div className="native-terminal-body"><aside className="native-symbol-sidebar">
        ...
        <section className="native-chart-panel">
          <div className="native-chart-toolbar">
            <div className="native-timeframes" aria-hidden="true" />
            <div className="native-chart-actions"><button title="Open GPT research laboratory" onClick={() => setResearchLabOpen(true)}><Beaker size={14} /></button></div>
          </div>
          <div className="native-chart-subbar"><div><b>{selected?.name || 'No instrument'}</b><span>{selected?.symbol || '—'}</span></div><div className="native-ohlc"><span>Bid <b>{latest?.bid !== undefined ? latest.bid.toLocaleString(undefined, { maximumFractionDigits: 8 }) : '—'}</b></span><span>Ask <b>{latest?.ask !== undefined ? latest.ask.toLocaleString(undefined, { maximumFractionDigits: 8 }) : '—'}</b></span></div><div>{latest ? latest.quote.toLocaleString(undefined, { maximumFractionDigits: 8 }) : '—'}</div></div>
          {lastError && <div className="native-chart-error">{lastError}</div>}
          <div className="native-chart-stage" aria-label="Chart workspace" />
        </section>
      </div>
