...
const TIMEFRAME_SECONDS: Record<Timeframe, number> = { tick: 0, '1s': 1, '5s': 5, '10s': 10, '15s': 15, '30s': 30, '1m': 60, '2m': 120, '3m': 180, '5m': 300, '10m': 600, '15m': 900, '30m': 1800, '1h': 3600, '2h': 7200, '4h': 14400, '6h': 21600, '8h': 28800, '12h': 43200, '1D': 86400, '1W': 604800, '1M': 2592000 };
...
function aggregateTicks(ticks: Tick[], timeframe: Timeframe): Candle[] {
  const map = new Map<number, Candle>();
  for (const tick of ticks.slice().sort((a, b) => a.epoch - b.epoch)) {
    const epoch = timeframe === 'tick' ? Math.floor(tick.epoch) : bucketEpoch(tick.epoch, timeframe);
...
export default function App() {
  ...
  const loadChartHistory = useCallback(async (symbol: string, frame: Timeframe) => {
    ...
    try {
      ...
      if (frame === 'tick' || seconds < 60) {
        ...
        const ticks: Tick[] = prices.map((price, index) => ({ symbol, quote: Number(price), epoch: Number(times[index]), source: 'Deriv' as const })).filter(tick => Number.isFinite(tick.quote) && Number.isFinite(tick.epoch));
        setCandles(aggregateTicks(ticks, frame));
      } else {
        const direct = frame !== '1W' && frame !== '1M';
        const data = await requestOnce(ws, { ticks_history: symbol, end: 'latest', count: direct ? 1000 : 1500, style: 'candles', granularity: direct ? seconds : 86400, adjust_start_time: 1 });
        const raw = Array.isArray(data.candles) ? data.candles as Array<Record<string, unknown>> : [];
        const daily = raw.map(candle => ({ epoch: Number(candle.epoch), open: Number(candle.open), high: Number(candle.high), low: Number(candle.low), close: Number(candle.close) })).filter(candle => Object.values(candle).every(Number.isFinite)).sort((a, b) => a.epoch - b.epoch);
        ...
        } else setCandles(daily.slice(-1000));
      }
    } catch (error) { setCandles([]); setLastError(error instanceof Error ? error.message : `Unable to load ${frame} history`); }
    finally { ws?.close(); setLoadingHistory(false); }
  }, []);
  ...
  const updateLiveCandle = useCallback((tick: Tick) => {
    ...
    setCandles(prev => {
      ...
      return [...prev, { epoch, open: tick.quote, high: tick.quote, low: tick.quote, close: tick.quote }].sort((a, b) => a.epoch - b.epoch).slice(-1500);
    });
  }, []);
  useEffect(() => {
    if (!selected) return;
      ...
      try {
        ...
        ws.addEventListener('message', event => {
          ...
          try {
            ...
            seen.current.add(key); liveBuffer.current.push(tick); setLatest(tick); updateLiveCandle(tick);
