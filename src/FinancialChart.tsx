  ...
  return new Promise((resolve, reject) => {
    ...
    ws.onmessage = event => {
      try {
        ...
        window.clearTimeout(timer); ws.close(); reject(error instanceof Error ? error : new Error('Invalid Deriv chart history'));
      }
    };
  });
}
...
export default function FinancialChart({ symbol, liveTick }: Props) {
  ...
  const candleState = useRef<Candle[]>([]);
  const selectedPeriod = useRef(60);
  const [period, setPeriod] = useState(60);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const liveRef = useRef<Tick | null>(null);

  useEffect(() => { liveRef.current = liveTick; }, [liveTick]);
  ...
  useEffect(() => {
    ...
    const chart = createChart(containerRef.current, {
      ...
      layout: { background: { type: ColorType.Solid, color: '#090d12' }, textColor: '#8995a5' },
  ...
  useEffect(() => {
    ...
    setLoading(true); setError(''); candleState.current = []; seriesRef.current.setData([]);
    requestHistory(symbol).then(ticks => {
      if (cancelled || !seriesRef.current) return;
      const candles = aggregate(ticks, selectedPeriod.current);
      candleState.current = candles;
      seriesRef.current.setData(candles);
      chartRef.current?.timeScale().fitContent();
    }).catch(error => { if (!cancelled) setError(error instanceof Error ? error.message : 'Unable to load chart history'); })
  ...
  useEffect(() => {
    ...
    const last = candleState.current[candleState.current.length - 1];
    const candle = last && last.time === bucket
    ...
    if (last && last.time === bucket) candleState.current[candleState.current.length - 1] = candle;
    else candleState.current.push(candle);
    if (candleState.current.length > 5000) candleState.current.shift();
  ...
  const changePeriod = (seconds: number) => {
    ...
    const candles = aggregate(
      candleState.current.map(c => ({ symbol, quote: c.close, epoch: Number(c.time) })),
      seconds,
    );
    candleState.current = candles;
    seriesRef.current?.setData(candles);
  ...
  return <div className="sire-financial-chart">
    <div className="sire-chart-controls">
      ...
      <button onClick={() => chartRef.current?.timeScale().scrollToRealtime()}>LIVE</button>
