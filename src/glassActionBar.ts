...
function installStyles() {
  ...
  style.textContent = `
    #${BAR_ID}{
      ...
      z-index:9998;
      left:50%;
      bottom:calc(max(6px,env(safe-area-inset-bottom)) + 57px);
    ...
    #${BAR_ID} .glass-action-label{overflow:hidden;text-overflow:ellipsis}
    @media(max-width:520px){
      ...
      #${BAR_ID} .glass-action{height:38px;padding:0 4px;gap:4px;font-size:8px;letter-spacing:.06em}
      #${BAR_ID} .glass-action-icon{font-size:14px}
    }
  `;
...
const observer = new MutationObserver(() => {
  if (!document.getElementById(BAR_ID)) mountBar();
});
