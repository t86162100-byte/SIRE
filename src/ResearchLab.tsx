...
type Props = { symbol: string; instruments: Instrument[]; onClose: () => void; onSelectInstrument?: (symbol: string) => void; onSetChartView?: (settings: Record<string, unknown>) => void; onAddMarker?: (label: string) => void };
...
type AgentResponse = { text?: string; responseId?: string; agentSkills?: string[]; error?: string; webSearched?: boolean; webSources?: WebSource[] };
...
export default function ResearchLab({ symbol, instruments, onClose, onSelectInstrument, onSetChartView, onAddMarker }: Props) {
  ...
  const runAgent = async (prompt: string, retrying = false) => {
    ...
    try { const lowerQuery = query.toLowerCase(); const directGptTest = lowerQuery.startsWith('/gpt '); const actualQuery = directGptTest ? query.slice(5).trim() : query; if (!actualQuery) throw new Error(directGptTest ? 'Use /gpt followed by a message.' : 'Message is required.'); const history = [...chatMessages.map(message => ({ role: message.role, text: message.text })), { role: 'user', text: actualQuery }];
      if (directGptTest) { const response = await api.post('/api/sire/agent/gpt', { query: actualQuery, history }); const raw = response as unknown; const data = ((raw && typeof raw === 'object' && 'data' in raw && (raw as Record<string, unknown>).data !== undefined ? (raw as Record<string, unknown>).data : raw) || {}) as AgentResponse; if (data.error) throw new Error(String(data.error)); if (isCancelled()) return; const reply = { id: `m-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, role: 'sire' as const, text: String(data.text || '').trim() || 'I’m here. Tell me more.' };
  ...
  return <div className="sire-chat-only-overlay"><section className="sire-chat-only" aria-label="SIRE conversation">
    ...
    <main className="sire-chat-only-messages"><div className="sire-chat-only-inner">
      ...
      {currentChatBusy && <article className="sire-council-live" aria-live="polite"><div className="sire-council-live-head"><span className="sire-council-live-pulse" /><strong>SIRE is working</strong><small>{thinkingSeconds < 2 ? 'starting' : `${thinkingSeconds}s`}</small><button type="button" className="sire-council-live-cancel" onClick={() => activeChatId && cancelConversation(activeChatId)}>Cancel</button></div><div className="sire-council-live-stream">{activity.length === 0 && <div className="sire-council-live-event"><span className="sire-council-live-actor">SIRE</span><span className="sire-council-live-phase">working</span><p>Working on your request…</p></div>}{activity.slice(-8).map((item, index) => <div className="sire-council-live-event" key={`${index}-${item.actor}-${item.phase}`}><span className="sire-council-live-actor">{item.actor}</span><span className="sire-council-live-phase">{phaseLabel(item.phase)}</span><p>{item.text}</p></div>)}<div className="sire-council-live-cursor"><i /><i /><i /></div></div></article>}
