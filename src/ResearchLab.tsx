...
export default function ResearchLab({ symbol, instruments, onClose, onSelectInstrument, onSetChartView, onAddMarker, runtimeContext }: Props) {
  ...
  useEffect(() => { try { const raw=window.localStorage.getItem('sire-issue-finder-logs'); const parsed=raw?JSON.parse(raw):[]; if(Array.isArray(parsed)) setIssueLogs(parsed); } catch {} }, []); const [thinkingSince, setThinkingSince] = useState<number | null>(null); const [thinkingSeconds, setThinkingSeconds] = useState(0); const [diagnosticReport, setDiagnosticReport] = useState<DiagnosticReport|null>(null);
