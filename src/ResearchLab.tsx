...
export default function ResearchLab({ symbol, instruments, onClose, onSelectInstrument, onSetChartView, onAddMarker, runtimeContext }: Props) {
  ...
  const runAgent = async (prompt: string, retrying = false) => {
    ...
    } catch (error) { if (isCancelled() || (error instanceof DOMException && error.name === 'AbortError')) return; const detail = error instanceof Error ? error.message : 'Connection failed'; setLastError(true); const reply = { id: `m-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, role: 'sire' as const, text: `I couldn’t complete that message. ${detail}`, meta: 'Retry available' }; persistChatMessage(chatId, reply); writeJob(chatId, 'error'); setProcessingChats(previous => previous.filter(id => id !== chatId)); if (runningChatIdRef.current === chatId) setChatSessions(previous => previous.map(chat => chat.id === chatId ? { ...chat, messages: [...chat.messages, reply].slice(-200), updatedAt: Date.now() } : chat)); } finally { if (retrying) setLastError(false); }
  };
