...
export default function ResearchLab({ symbol, instruments, onClose, onSelectInstrument, onSetChartView, onAddMarker, runtimeContext }: Props) {
  ...
  const [diagnosticsOpen, setDiagnosticsOpen] = useState(false); const [diagnosticsBusy, setDiagnosticsBusy] = useState(false); const [diagnosticReport, setDiagnosticReport] = useState<DiagnosticReport|null>(null);
  ...
  useEffect(() => { runtimeContextRef.current = runtimeContext || null; }, [runtimeContext]); useEffect(() => { setActiveSymbol(symbol); }, [symbol]); useEffect(() => { chatEndRef.current?.scrollIntoView({ behavior: 'smooth', block: 'end' }); }, [chatMessages, currentChatBusy, activity, webSources]);
  ...
  const runAgent = async (prompt: string, retrying = false) => {
    ...
    } catch (error) { if (isCancelled() || (error instanceof DOMException && error.name === 'AbortError')) return; const detail = error instanceof Error ? error.message : 'Connection failed'; setLastError(true); const reply = { id: `m-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, role: 'sire' as const, text: `I couldn’t complete that message. ${detail}`, meta: 'Retry available' }; persistChatMessage(chatId, reply); writeJob(chatId, 'error'); setProcessingChats(previous => previous.filter(id => id !== chatId)); setChatSessions(previous => previous.map(chat => chat.id === chatId ? { ...chat, messages: [...chat.messages, reply].slice(-200), updatedAt: Date.now() } : chat)); } finally { if (retrying) setLastError(false); }
  };
  ...
  return <div className="sire-chat-only-overlay"><section className="sire-chat-only" aria-label="SIRE conversation">
    ...
    <main className="sire-chat-only-messages"><div className="sire-chat-only-inner">
      ...
      {currentChatBusy && <article className="sire-council-live" aria-live="polite"><div className="sire-council-live-head"><span className="sire-council-live-pulse" /><strong>SIRE is thinking</strong><small>live</small><button type="button" className="sire-council-live-cancel" onClick={() => activeChatId && cancelConversation(activeChatId)}>Cancel</button></div><div className="sire-council-live-stream">{activity.slice(-8).map((item, index) => <div className="sire-council-live-event" key={`${index}-${item.actor}-${item.phase}`}><span className="sire-council-live-actor">{item.actor}</span><span className="sire-council-live-phase">{phaseLabel(item.phase)}</span><p>{item.text}</p></div>)}<div className="sire-council-live-cursor"><i /><i /><i /></div></div></article>}
