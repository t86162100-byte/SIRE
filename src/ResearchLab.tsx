...
type Props = { symbol: string; instruments: Instrument[]; onClose: () => void; onSelectInstrument?: (symbol: string) => void };
type ChatMessage = { id: number; role: 'user' | 'sire'; text: string; meta?: string };
type AgentResponse = { text?: string; responseId?: string; actions?: Array<Record<string, unknown>> };
...
function RichMessage({ text }: { text: string }) {
  ...
  let paragraph: string[] = []; let list: string[] = []; let ordered = false; let quote: string[] = []; let code: string[] = []; let inCode = false;
  ...
  lines.forEach(line => {
    if (line.trim().startsWith('```')) { if (inCode) flushCode(); else { flushParagraph(); flushList(); flushQuote(); } inCode = !inCode; return; }
...
export default function ResearchLab({ symbol, instruments, onClose, onSelectInstrument }: Props) {
  ...
  const copyMessage = async (message: ChatMessage) => {
    try { await navigator.clipboard.writeText(message.text); setCopiedId(message.id); window.setTimeout(() => setCopiedId(current => current === message.id ? null : current), 1400); } catch { /* native text selection remains available */ }
  };
  ...
  const runAgent = async (prompt: string) => {
    ...
    try {
      ...
      actions.forEach(action => {
        if (String(action.__sireAction || '') !== 'select_instrument') return;
      ...
      const detail = error instanceof Error ? error.message : 'Connection failed'; setLastError(true);
  ...
  return <div className="sire-chat-only-overlay"><section className="sire-chat-only" aria-label="SIRE conversation">
    <header className="sire-chat-only-header"><div className="sire-chat-only-brand"><div className="sire-chat-only-mark"><Sparkles size={16} /></div><span>SIRE</span><i /></div><button className="sire-chat-only-close" onClick={onClose} aria-label="Close SIRE"><X size={18} /></button></header>
    <main className="sire-chat-only-messages"><div className="sire-chat-only-inner">
      ...
      {chatMessages.map(item => <article className={`sire-chat-only-message ${item.role}`} key={item.id}><div className="sire-chat-only-role">{item.role === 'sire' ? 'SIRE' : 'YOU'}</div><div className="sire-chat-only-message-row"><div className="sire-chat-only-bubble">{item.role === 'sire' ? <RichMessage text={item.text} /> : <div className="sire-user-content">{item.text}</div>}</div><button className="sire-chat-only-copy" onClick={() => void copyMessage(item)} aria-label={copiedId === item.id ? 'Copied' : 'Copy message'}>{copiedId === item.id ? <Check size={14} /> : <Copy size={14} />}</button></div>{item.meta && <small>{item.meta}</small>}</article>)}
      ...
      {lastError && !chatBusy && <button className="sire-chat-only-retry" onClick={() => void runAgent(lastPrompt)}>Retry</button>}
      <div ref={chatEndRef} />
    </div></main>
