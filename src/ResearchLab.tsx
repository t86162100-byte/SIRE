...
export default function ResearchLab({ symbol, instruments, onClose, onSelectInstrument, onSetChartView, onAddMarker, runtimeContext }: Props) {
  ...
  return <div className="sire-chat-only-overlay"><section className="sire-chat-only" aria-label="SIRE conversation">
    ...
    <main className="sire-chat-only-messages"><div className="sire-chat-only-inner">
      ...
      {chatMessages.map(item => <article className={`sire-chat-only-message ${item.role}`} key={item.id}><div className="sire-chat-only-bubble">{item.role === 'sire' ? <RichMessage text={item.text} /> : <div className="sire-user-text">{item.text}</div>}</div>{item.role === 'user' && currentChatBusy && item.id === chatMessages[chatMessages.length - 1]?.id && <div className="sire-inline-live-anchor"><span className="sire-inline-live-star">✦</span><span className="sire-inline-live-text">{activity[activity.length - 1]?.text || 'Reading your request…'}</span></div>}{item.role === 'sire' && <MessageActions text={item.text} />}{item.meta && <small>{item.meta}</small>}</article>)}}
      {currentChatBusy && <div className="sire-inline-live" aria-live="polite"><span className="sire-inline-live-star">✦</span><span className="sire-inline-live-text">{activity[activity.length - 1]?.text || 'Reading your request…'}</span></div>}
